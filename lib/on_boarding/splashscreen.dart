...
class _SplashScreenState extends State<SplashScreen> {
  ...
  Future<void> tonext() async {
    ...
    if (!await _ensureBluetoothService()) {
      _exitApp();
    }

    // بدء التشييك المستمر كل ثانية
    _locationCheckTimer = Timer.periodic(const Duration(seconds: 1), (
      timer,
    ) async {
  ...
  Future<bool> _ensureBluetoothService() async {
    // نتحقق من حالة البلوتوث
    BluetoothState state = await FlutterBluetoothSerial.instance.state;

    // لو البلوتوث مش شغال
    if (state != BluetoothState.STATE_ON) {
      // نطلب من النظام يطلع البوب-أب لتشغيله
    ...
    return true; // البلوتوث شغال بالفعل أو اتشغل دلوقتي
  }
